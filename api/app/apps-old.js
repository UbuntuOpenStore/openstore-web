            ...
            if (err) {
                ...
                if (req.file.originalname.indexOf('.click') == -1 && req.file.originalname.indexOf('.snap') == -1) {
                    ...
                    if (helpers.isAdminOrTrustedOwner(req, pkg)) {
                        ...
                        var file_path = req.file.path + '.click'; //TODO handle clicks
                        //TODO only check for manual review for non admin/trusted users
                        fs.rename(req.file.path, file_path, (err) => {
                            if (err) {
                                helpers.error(res, 'Failed to update app');
                            }
                            else {
                                exec(config.clickreview.command + ' --json ' + file_path, {PYTHONPATH: config.clickreview.pythonpath}, function callback(err, stdout, stderr){
                                    if (err) {
                                        logger.error('Failed to review app:', err, stderr);
                                    }
                                    else {
                                        var review = JSON.parse(stdout);
                                        var manualReview
                                        for (var key in review) {
                                            for (var level in review[key]) {
                                                for (var label in review[key][level]) {
                                        ...
                                        //TODO throw an error for manual review

                                        pkg.updated_date = moment().toISOString();
                                        packages.updateInfo(pkg, null, req.body, req.file, null, function(pkg) {
                                            parseFile(pkg, file_path, function(err) {
                                                if (err) {
                                                    helpers.error(res, err);
                                                }
                                                else {
